...
class TestPymodulegenCLI(unittest.TestCase):
    ...
    def test_help_command(self):
        ...
        result = subprocess.run(['python', 'src\pymodulegen\__main__.py', '--help'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    ...
    def test_create_module_1(self):
        ...
        cmd = ['python', 'src\pymodulegen\__main__.py', 'some_module_1', '--is_module_only']
    ...
    def test_create_package_1(self):
        """
		src\pymodulegen\__main__.py authentication.py --directory "api" --is_module_only
        ...
        try:
            ...
            assert os.path.exists('api/authentication.py'), "Module file 'api/authentication.py' was not created."

            #Check if the file content is empty
            file_size = os.path.getsize('api/authentication.py')
            assert file_size == 0, "Module file 'api/authentication.py' is not empty."
			
			# Check if the file 'api/__init__.py' exists
            assert os.path.exists('api/__init__.py'), "Module file 'api/__init__.py' was not created."
            ...
            with open('api/__init__.py','r') as init_file:
    ...
    def test_create_package_2(self):
        ...
        """
        # Define the command to run
        cmd = ['python', 'src\pymodulegen\__main__.py', 'authentication','--directory','api','--not_is_module_only']
        ...
        try:
            ...
            directory = (os.getcwd().split(os.sep))[-1]
            expected_content=f"root_path = os.path.join(current_path.split(os.sep + '{directory}')[0], '{directory}')"
            with open('api/authentication.py','r') as init_file:
                file_content = init_file.read();
